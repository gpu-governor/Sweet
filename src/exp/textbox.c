...
void render_text_box(CREATE *textBox) {
    ...
    if (textBox->active && textBox->show_cursor) {
        // Cursor should start at the padding offset, regardless of placeholder
        int cursor_x;
        if (strlen(textBox->text) == 0) {
            cursor_x = textBox->rect1.x + textBox->padding;  // Start at the beginning if no text
        } else {
            cursor_x = textBox->rect1.x + textBox->padding + text_width;  // Cursor after text
        }
...
int main(int argc, char* argv[]) {
   ...
   // CREATE textBox = sw_text_box(100, 100, 300, 50, "Enter text here...", 24, textColor, placeholderColor, bgColor, NORMAL);
	CREATE textBox=sw_text_box(150, 150, 300, 50, "Enter text here...",24, textColor, placeholderColor, bgColor, outlineColor, outlineColorSelected, NORMAL);
